...
selected_brands = st.sidebar.multiselect("选择品牌（可多选）", options=all_brands, default=["VERSACE"])
store_type_filter = st.sidebar.multiselect("门店类型", options=["All"] + sorted(df_stores["type"].unique()), default=["All"])
operator_filter = st.sidebar.multiselect("运营商", options=["All"] + sorted(df_stores["operator"].unique()), default=["All"])
...
col1.metric("🏪 总门店数", len(df_stores))
col2.metric("📍 覆盖城市", df_stores["city"].nunique())
col3.metric("🗺️ 覆盖省级区域", df_stores["province"].nunique())
col4.metric("🏷️ 品牌总数", len(all_brands))
st.markdown("---")
...
if selected_brands:
    ...
    for i, brand in enumerate(selected_brands):
        with brand_cols[i]:
            ...
            st.info(f"**{brand}**\n\n门店: {info['store_count']} 家\n区域: {len(info['provinces'])} 个")

    c1, c2, c3 = st.columns(3)
    c1.metric("共同覆盖区域（交集）", len(covered_all))
    c2.metric("任一品牌覆盖区域（并集）", len(union_all))
    c3.metric("全域覆盖率", f"{round(len(covered_all) / len(all_regions) * 100, 1)}%")

st.markdown("---")

# ============================================================
# 7. 中国地图 — ✨ 单个Choropleth修复版
# ============================================================
st.subheader("📍 中国门店分布与品牌覆盖地图")
fig_map = go.Figure()

# ---- 7a. 单层 Choropleth：z值区分颜色 ----
...
fig_map.add_trace(go.Scattergeo(
    lon=df_map_stores["lng"], lat=df_map_stores["lat"],
    text=hover_texts_marker, hoverinfo="text", mode="markers",
    marker=dict(symbol=marker_symbols, size=11, color=marker_colors,
...
fig_map.add_trace(go.Scattergeo(
    lon=prov_lngs, lat=prov_lats, text=prov_labels,
    mode="markers+text",
...
fig_map.update_layout(
    ...
    geo=dict(
        scope="asia", projection=dict(type="natural earth"),
    ...
    height=750, margin=dict(l=10, r=10, t=60, b=20), showlegend=False,
)
...
tab1, tab2, tab3, tab4 = st.tabs(["门店类型分布", "运营商分布", "区域门店排行", "品牌覆盖排行"])

with tab1:
    tc = df_stores["type"].value_counts().reset_index()
    tc.columns = ["门店类型", "数量"]
    fig1 = px.pie(tc, values="数量", names="门店类型", title="门店类型分布",
...
with tab2:
    ...
    oc.columns = ["运营商", "数量"]
    fig2 = px.bar(oc, x="运营商", y="数量", title="运营商门店数量排行",
...
with tab3:
    ...
    pc.columns = ["区域", "门店数"]
    fig3 = px.bar(pc.sort_values("门店数", ascending=True),
                  x="门店数", y="区域", title="各省/区域门店数量排行（含港澳）",
...
with tab4:
    ...
    for brand, info in BRAND_COVERAGE.items():
        bd.append({"品牌": brand, "覆盖区域数": len(info["provinces"]), "门店数": info["store_count"]})
    df_bd = pd.DataFrame(bd).sort_values("覆盖区域数", ascending=True)
    fig4 = px.bar(df_bd, x="覆盖区域数", y="品牌", title="品牌覆盖区域数量排行",
